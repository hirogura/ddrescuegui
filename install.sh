...
install_deps_debian() {
    apt-get update -qq
    apt-get install -y -qq python3 gddrescue smartmontools fdisk git clonezilla partclone
}

install_deps_arch() {
    # CachyOS / Arch 系のパッケージ名に読み替える
    #   gddrescue -> ddrescue, fdisk -> util-linux (lsblk/blkid/fdisk 同梱), file 追加
    pacman -Sy --noconfirm --needed python ddrescue smartmontools util-linux file git clonezilla partclone
}
...
if [ -n "$(echo " $OS_LIKE " | grep -i " arch ")" ] || [ "$OS_ID" = "arch" ] || [ "$OS_ID" = "cachyos" ]; then
    ...
    echo "Please install manually: python3, ddrescue(gddrescue), smartmontools, fdisk(util-linux), git, clonezilla, partclone" >&2
    exit 1
fi
