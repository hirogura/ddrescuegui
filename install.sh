...
install_deps_arch() {
    ...
    pacman -Sy --noconfirm --needed python ddrescue smartmontools util-linux file git
}
...
if [ -n "$(echo " $OS_LIKE " | grep -i " arch ")" ] || [ "$OS_ID" = "arch" ] || [ "$OS_ID" = "cachyos" ]; then
    ...
    echo "Please install manually: python3, ddrescue(gddrescue), smartmontools, fdisk(util-linux), git" >&2
    exit 1
fi
